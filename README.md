...
Set `GITHUB_BUG_REPORT_REPO` if you want reports to land somewhere other than `leandromesq/unburden`.
...
- `leandromesq/unburden/.github/workflows/issues-to-discord.yml` forwards app-created GitHub bug reports to Discord when new issues open
...
- `@pkmn/dex` and `@pkmn/mods/champions` for Pokemon Champions species, moves, abilities, learnsets, items, and regulation snapshots
- Smogon Champions VGC usage statistics for competitive usage/meta

Runtime external assets:

- Pokemon Showdown sprite URLs generated by `@pkmn/img`
