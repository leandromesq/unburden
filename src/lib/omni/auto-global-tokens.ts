...
import { getPokemonAbilityNames } from "@/lib/domain/prompt-resolution";
...
import type { ImportedSet, SuggestionOption } from "@/lib/types";
...
function resolveScopedAbilityName(
	...
	if (explicitAbility) {
		...
		const knownAbilities = buildCommonAbilities(
			profile,
			getPokemonAbilityNames(pokemonId),
		);
