...
function resolveCalcSpeciesName(
	pokemon: {
		id: string;
		name: string;
	...
	for (const candidateId of candidateIds) {
		const species = GEN_9.species.get(
			candidateId as Parameters<typeof GEN_9.species.get>[0],
		);
...
function buildAppliedStageBoosts(parsed: ParsedCommand, moveCategory: string) {
	...
	const attackerStageStatKey = resolveAttackingStatKey(
		parsed.move,
		moveCategory,
	);
...
function isRelevantAbility(
	...
	if (
		moveType === "Fire" &&
		new Set(["flashfire", "wellbakedbody", "heatproof", "thickfat"]).has(
			normalized,
		)
	) {
		return true;
	...
	if (
		moveType === "Water" &&
		new Set(["dryskin", "stormdrain", "waterabsorb"]).has(normalized)
	) {
		return true;
	}
	...
	if (
		moveType === "Electric" &&
		new Set(["lightningrod", "motordrive", "voltabsorb"]).has(normalized)
	) {
		return true;
	}
...
function describeAssumptions(
	...
	if (parsed.attackerStatus) {
		assumptions.push(
			`Attacker status: ${formatStatusLabel(parsed.attackerStatus)}`,
		);
	}

	if (parsed.defenderStatus) {
		assumptions.push(
			`Defender status: ${formatStatusLabel(parsed.defenderStatus)}`,
		);
	}

	if (attackerSet) {
		assumptions.push(
			`Set SPs: ${attackerSet.statPoints.hp}/${attackerSet.statPoints.atk}/${attackerSet.statPoints.def}/${attackerSet.statPoints.spa}/${attackerSet.statPoints.spd}/${attackerSet.statPoints.spe}`,
		);
	}

	if (defenderSet) {
		assumptions.push(
			`Defender SPs: ${defenderSet.statPoints.hp}/${defenderSet.statPoints.atk}/${defenderSet.statPoints.def}/${defenderSet.statPoints.spa}/${defenderSet.statPoints.spd}/${defenderSet.statPoints.spe}`,
		);
	}

	if (
		attackerAbility &&
		(attackerAbilitySource !== "default" ||
			isRelevantAbility(
				attackerAbility,
				"attacker",
				moveType,
	...
	if (parsed.fickleBeamDouble) {
		assumptions.push("Fickle Beam: double damage (30% chance)");
	}

	if (parsed.roundDouble) {
		assumptions.push(
			"Round: double damage (another Pok&eacute;mon used Round)",
		);
	}

	if (parsed.rageFistHits !== undefined) {
		assumptions.push(
			`Rage Fist: ${parsed.rageFistHits} hit(s) taken (${Math.min(50 + 50 * parsed.rageFistHits, 350)} BP)`,
		);
	}

	if (parsed.moveHitCount && resolvedMoveHitCount && resolvedMoveHitCount > 1) {
		assumptions.push(`Hits: ${resolvedMoveHitCount}`);
	} else if (
		isVariableHitMove &&
		resolvedMoveHitCount &&
		resolvedMoveHitCount > 1
	) {
...
export function buildCalculationContext(
	...
	const resolvedAttacker =
		!parsedAttacker.isMega && attackerItem
			? (resolveMegaEvolution(parsedAttacker.id, attackerItem) ??
				parsedAttacker)
			: parsedAttacker;
	const resolvedDefender =
		!parsedDefender.isMega && defenderItem
			? (resolveMegaEvolution(parsedDefender.id, defenderItem) ??
				parsedDefender)
			: parsedDefender;
	...
	const attackerPokemonResult = createCalcPokemon(
		attacker,
		"attacker",
		{
			...attackerBaseOptions,
	...
	const speedRatio =
		normalizeId(move.name) === "electroball"
			? attackerSpeedContext.effectiveSpeed > 0 &&
				defenderSpeedContext.effectiveSpeed > 0
				? attackerSpeedContext.effectiveSpeed /
					defenderSpeedContext.effectiveSpeed
				: undefined
...
export function calculateDamageResults(
	...
	return context.archetypes.flatMap((archetype): DamageResult[] => {
		...
		const defenderPokemonResult = createCalcPokemon(
			context.defender,
			"defender",
			{
				...defenderBaseOptions,
