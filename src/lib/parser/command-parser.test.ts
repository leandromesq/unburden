...
describe("parseCommand", () => {
	...
	test("parses Rage Fist hits, Fickle Beam and Round double from move symbols", () => {
		const fickleDouble = parseCommand("dragapult !fickle-beam[1] x incineroar");
		const fickleNormal = parseCommand("dragapult !fickle-beam x incineroar");
		const roundDouble = parseCommand("sylveon !round[1] x incineroar");

		expect(fickleDouble.parsed?.fickleBeamDouble).toBe(true);
		expect(fickleNormal.parsed?.fickleBeamDouble).toBeUndefined();
		expect(roundDouble.parsed?.roundDouble).toBe(true);
	});

	test("forces trick room into parsed global effects", () => {
		const result = parseCommand(
			"charizard !heat-wave ~trick-room x incineroar",
		);
	...
	test("parses attacker and defender hp percentages plus critical hit", () => {
		const result = parseCommand("politoed !muddy-water %75 * x incineroar %50");
	...
	test("resolves Floette Eternal Flower as a distinct species", () => {
		const result = parseCommand(
			"floette eternal flower !moonblast x incineroar",
		);
	...
	test("supports explicit Aegislash blade and shield forms as calc overrides", () => {
		const result = parseCommand(
			"aegislash blade !poltergeist x aegislash shield",
		);
