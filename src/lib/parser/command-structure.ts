...
function parseMoveSymbol(raw: string) {
	...
	const isRageFist = value === "rage-fist";
	const isFickleBeam = value === "fickle-beam";
	const isRound = value === "round";
	const isSpecialParameter =
		isLastRespects || isRageFist || isFickleBeam || isRound;
	...
	const rageFistHits =
		isRageFist && parsedParameter !== undefined ? parsedParameter : undefined;
	const rageFistHitsInvalid =
		rageFistHits !== undefined &&
		(!Number.isInteger(rageFistHits) || rageFistHits < 0 || rageFistHits > 6);
	const fickleBeamDouble =
		isFickleBeam && parsedParameter === 1 ? true : undefined;
	const roundDouble = isRound && parsedParameter === 1 ? true : undefined;
	const parsedHitCount =
		!isSpecialParameter && parsedParameter !== undefined
			? parsedParameter
			: undefined;
	...
	return {
		...
		lastRespectsStacks: lastRespectsStacksInvalid
			? undefined
			: lastRespectsStacks,
		rageFistHits: rageFistHitsInvalid ? undefined : rageFistHits,
		fickleBeamDouble: fickleBeamDouble ?? undefined,
		roundDouble: roundDouble ?? undefined,
	};
}
...
function parseExplicitSymbolToken(
	...
	if (token.normalized.startsWith("m:") || token.normalized.startsWith("!")) {
		...
		return {
			...
			rageFistHits: parsedMove.rageFistHits,
			fickleBeamDouble: parsedMove.fickleBeamDouble,
			roundDouble: parsedMove.roundDouble,
			source: token,
		};
	}
	...
	if (token.normalized.startsWith("~")) {
		return {
			...
			kind: "modifier",
			scope: "global",
			value: normalizeModifierValue(token.raw.slice(1)),
			source: token,
		};
	}
...
function parseBareSegmentToken(
	...
	if (side === "attacker" && ATTACKER_MODIFIER_MAP.has(normalizedValue)) {
		return {
			raw: token.raw,
			normalized: token.normalized,
			kind: "modifier",
	...
	if (side === "defender" && DEFENDER_MODIFIER_MAP.has(normalizedValue)) {
		return {
			raw: token.raw,
			normalized: token.normalized,
			kind: "modifier",
...
function analyzeSegment(
	...
	const itemToken = symbolTokens
		.filter((token) => token.kind === "item")
		.at(-1);
	const abilityToken = symbolTokens
		.filter((token) => token.kind === "ability" && token.scope === side)
		.at(-1);
	const statPointToken = symbolTokens
		.filter((token) => token.kind === "stat_points" && token.scope === side)
		.at(-1);
	const hpToken = symbolTokens.filter((token) => token.kind === "hp").at(-1);
	const criticalToken = symbolTokens
		.filter((token) => token.kind === "critical")
		.at(-1);
	...
	return {
		...
		leadingFreeTokens,
		speciesTokens:
			exact || hasLeadingReference ? speciesTokens : leadingFreeTokens,
		speciesText,
		speciesExact: exact,
		speciesMatch:
			speciesText && !hasLeadingReference
				? resolvePokemonEntity(speciesText)
				: null,
...
export function isKnownModifierToken(
	scope: "attacker" | "defender" | "global",
	value: string,
) {
	if (scope === "attacker") {
